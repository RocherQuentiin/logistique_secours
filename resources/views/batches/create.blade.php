...
<form method="POST" action="{{ route('batches.store') }}" class="space-y-4 max-w-xl">@csrf
  ...
  <div><label class="block text-sm">Nom du lot</label><input name="name" class="w-full border rounded px-2 py-1" required></div>
  ...
  <div><label class="block text-sm">Quantité</label><input type="number" name="quantity" min="0" class="w-full border rounded px-2 py-1" required></div>
  <div><label class="block text-sm">Date de péremption</label><input type="date" name="expiry_date" class="w-full border rounded px-2 py-1"></div>
